...
class NekoMain
{
	...
	static function get_sample_info(demux_res : DemuxResT, samplenum: Int, sampleinfo : SampleDuration) : Int
	{
		...
		{
			neko.Lib.println("sample " + samplenum + " does not exist ");
			return 0;
		}

		if (demux_res.num_time_to_samples == 0)		// was null
		{
			neko.Lib.println("no time to samples");
			return 0;
		}
		while ((demux_res.time_to_sample[duration_cur_index].sample_count + duration_index_accum) <= samplenum)
		{
			...
			{
				neko.Lib.println("sample " + samplenum + " does not have a duration");
				return 0;
			}
		}
	...
	static function setup_environment(argc : Int, argv : Array <String>) : Void
	{
		...
		while (arg_idx < neko.Sys.args().length)
		{
			...
			{
				if (StringTools.startsWith(neko.Sys.args()[arg_idx], "-r") || StringTools.startsWith(neko.Sys.args()[arg_idx], "-R") )
				{
					// raw PCM output
					write_wav_format = 0;
				}
				if (StringTools.startsWith(neko.Sys.args()[arg_idx], "-v") || StringTools.startsWith(neko.Sys.args()[arg_idx], "-V") )
				{
					// verbose
					verbose = 1;
				}
				if (StringTools.startsWith(neko.Sys.args()[arg_idx], "-t") || StringTools.startsWith(neko.Sys.args()[arg_idx], "-T") )
				{
					// test file type
					test_file_type = 1;
				}
			...
			{
				input_file_n = neko.Sys.args()[arg_idx];
			}
			else if (output_file_n.length == 0)
			{
				output_file_n = neko.Sys.args()[arg_idx];
			}
			else
			{
		...
		if (output_file_n.length != 0)
		{
			...
			output_stream = neko.io.File.write(output_file_n,true);

			output_opened = 1;
		}

		input_stream = neko.io.File.read(input_file_n,true);
	...
	static function GetBuffer(demux_res : DemuxResT) : Void
	{
		...
		for(i in 0 ... demux_res.num_sample_byte_sizes)
		{
			...
			{
				neko.Lib.println("sample failed");
				return;
			}
			...
			{
				neko.Lib.println("sorry buffer too small! (is " + buffer_size + " want " + sample_byte_size + ")");
				return;
			}
			...
			if (verbose != 0)
				neko.Lib.println("read " + outputBytes + " bytes. total: " + bytes_read);
		...
		if (verbose != 0)
			neko.Lib.println("done reading, read " + i + " frames");
	}
	...
	{
		neko.Lib.println("Usage: alac [options] inputfile outputfile");
		neko.Lib.println("Decompresses the ALAC file specified");
		neko.Lib.println("Options:");
		neko.Lib.println("  -r                write output as raw PCM data. Default");
		neko.Lib.println("                    is in WAV format.");
		neko.Lib.println("  -v                verbose output.");
		neko.Lib.println("  -t                test that file is ALAC, also tests for");
		neko.Lib.println("                    other m4a file types.");
		neko.Lib.println("");
		neko.Lib.println("This port of the code is (c) Peter McQuillan 2011");
		neko.Lib.println("Original software is (c) 2005 David Hammerton");
		neko.Sys.exit(1);
	}
	...
	public static function main()
	{
		...
		setup_environment(neko.Sys.args().length, neko.Sys.args());

		if (input_stream == null)
		{
			neko.Lib.println("failed to create input stream from file");
			return 1;
		}
		...
		if (headerRead == 0)
		{
			...
			{
				neko.Lib.println("failed to load the QuickTime movie headers");
				if (demux_res.format_read != 0)
					 neko.Lib.println("file type: " + demux_res.format);
				else
					 neko.Lib.println("");
				return 1;
			}
		...
		if (test_file_type != 0)
		{
			...
			if (demux_res.format_read == 0)
			{
				neko.Lib.println("failed to load the QuickTime movie headers." + " Probably not a quicktime file");
				return 1;
			}
			neko.Lib.println("file type: " + demux_res.format);
